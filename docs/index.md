...
Goop shows a pairing QR code as soon as the signer dialog opens. Scan it in a compatible NIP-46 signer (such as Amber) and approve. Goop connects automatically and remembers the connection for next time. Codes expire after two minutes; click the refresh icon to generate another. Refreshing or opening the scanner keeps earlier invitations active until they expire. Approving any invitation connects Goop and cancels the others. **Escape** cancels pairing and opens URL entry.
