...
impl Render for BlockedUsers {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex().size_full().p_4().gap_4().overflow_y_scrollbar()
            .child(div().text_sm().text_color(cx.theme().text_muted).child("Blocked users are hidden from Inbox and Requests. This list syncs with compatible Nostr apps."))
            .when(pending, |view| view.child("Changes are saved locally and waiting to sync."))
            ...
            .when(keys.is_empty(), |view| view.child("No blocked users."))
            .children(keys.into_iter().map(|key| {
                let profile = PersonRegistry::global(cx).read(cx).get(&key,cx);
                v_flex().gap_2().pb_3().border_b_1().border_color(cx.theme().border)
                    .child(h_flex().justify_between().gap_2()
                        .child(Button::new(format!("profile-{key}")).label(profile.name()).ghost().on_click(move |_,window,cx| window.dispatch_action(Box::new(crate::Command::OpenProfile(key)),cx)))
                        .child(Button::new(format!("unblock-{key}")).label("Unblock").small().ghost().on_click(move |_,window,cx| crate::dialogs::moderation::block(key,window,cx))))
                    .child(div().text_xs().truncate().text_color(cx.theme().text_muted).child(key.to_bech32().unwrap()))
            }))
    }
}
